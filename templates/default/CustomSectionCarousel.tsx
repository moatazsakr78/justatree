    ...
    <div className={`relative ${className}`}>
            ...
            >
              ...
              {hasClones(product) ? (
                <button
                  ...
                  style={{backgroundColor: expandedProductId === String(product.id) ? 'var(--primary-hover-color)' : 'var(--primary-color)'}}
                  onMouseEnter={(e) => {
                  ...
                  onMouseLeave={(e) => {
                    (e.target as HTMLButtonElement).style.backgroundColor = expandedProductId === String(product.id) ? 'var(--primary-hover-color)' : 'var(--primary-color)';
                    (e.target as HTMLButtonElement).style.boxShadow = 'none';
                  }}
                >
                  {expandedProductId === String(product.id) ? 'إخفاء الأشكال' : 'اختر الشكل'}
                </button>
              ) : (
                <button
                  onClick={async (e) => {
      ...
      {expandedProduct && expandedProduct.clones && expandedProduct.clones.length > 0 && (
        ...
        >
          ...
          {/* Clone Grid - Same size as product cards */}
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                >
                  <div className="overflow-hidden">
                    <img
                      ...
                      className="w-full h-72 object-cover group-hover/clone:scale-105 transition-transform duration-300"
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        if (target.src !== '/placeholder-product.svg') {
                          target.src = '/placeholder-product.svg';
                  ...
                  <div className="p-4">
                    {clone.label && (
                      <p className="text-sm font-medium text-gray-700 truncate mb-3 text-center">{clone.label}</p>
                    )}
                    <button
                      onClick={() => handleCloneAddToCart(expandedProduct, clone)}
                      className="w-full px-4 py-2 rounded-lg text-sm font-medium text-white transition-all hover:scale-105 active:scale-95"
                      style={{backgroundColor: 'var(--primary-color)'}}
                      onMouseEnter={(e) => {
                        (e.target as HTMLButtonElement).style.backgroundColor = 'var(--primary-hover-color)';
                      }}
